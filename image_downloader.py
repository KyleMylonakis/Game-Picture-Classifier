...
def main():
    ...
    for ii in range(0,number_of_games):
        ...
        for url in game_df['URLs']:
            if not os.path.exists(download_path +'/raw_images/' + str(games_df.at[ii,'App ID']).replace(" ", "_").replace(":", "-")):
                print('Creating Raw Image Folder')
                os.makedirs(download_path +'/raw_images/' + str(games_df.at[ii,'App ID']).replace(" ", "_").replace(":", "-"))
            if num_pics < pics_per_game:
                img_url = url
                img = requests.get(img_url).content
                with open(download_path +'/raw_images/' + str(games_df.at[ii,'App ID']).replace(" ", "_").replace(":", "-") +'/' +str(num_pics) + '.png', 'wb' ) as handler:
                    handler.write(img)
                num_pics +=1
